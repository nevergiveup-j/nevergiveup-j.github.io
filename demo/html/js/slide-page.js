...
;(function (factory) {
    ...
    SlidePage.prototype.touchStart = function(event) {
        ...
        if(this.isSrcoll){
            // pageNow
            this.$pageSrcoll = this.$pages.eq(this.opts.pageNow).find('.page-scroll-height');
    ...
    SlidePage.prototype.touchMove = function(event) {
        ...
        // 当前页面有滚动条
        if ( this.isSrcoll ) {
            var viewHeight = this.$pageSrcoll.find('.page-scroll-height').height() - $(window).height() - 10;

            if ( opts.moveDirection == 'up' ) {
                if ( this.startScrollTop < viewHeight ) {
                    return;
                }
            } else {
    ...
    SlidePage.prototype.togglePage = function(status) {
        ...
        setTimeout(function() {
            ...
            // 页面是否有滚动条
            if ( that.$pages.eq( that.opts.pageNext).attr('data-scroll') ) {
                that.isSrcoll = true;
            } else {
                that.isSrcoll = false;
            }
